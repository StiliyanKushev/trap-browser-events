...
namespace TrapBrowserEvents
{
    public static class HookManager
    {
        ...
        public delegate void HookEventHandler(WinEvents eventType, IntPtr hwnd);
        public static event HookEventHandler hookEventHandler;

        public static bool handlersInitialized = false;

        public static void SubscribeToWindowEvents()
        {
            // make sure event loop is alive
            EventLoop.Run(() =>
            {
                if(!handlersInitialized) 
                {
                    handlersInitialized = true;
                    Listeners.InitializeHookEventListeners();
                }

                if (windowEventHook == IntPtr.Zero)
                {
                    windowEventHook = SetWinEventHook(
                        WinEvents.EVENT_MIN,  // eventMin
                        WinEvents.EVENT_MAX,  // eventMax
        ...
        private static void WindowEventCallback(
            ...
            uint dwmsEventTime)
        {
            hookEventHandler?.Invoke(eventType, hwnd);
        }
...
*/
    ...
    public static class EventLoop
    {
        ...
        public static void Run(Action MessagePump)
        {
